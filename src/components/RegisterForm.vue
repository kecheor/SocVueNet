<template>
  <div>
    <b-form @submit="onSubmit" novalidate >
      ...
      <b-alert variant="danger" :show="error.length === 0 ? null : true" dismissible>
       {{error}}
      </b-alert>
      <b-form-group>
        <b-button type="submit" :variant="(formValid ? 'success' : 'outline-dark')" :disabled="!formValid">Register</b-button>
      </b-form-group>
    </b-form>
  </div>
</template>
...
export default {
  ...
  computed: {
    loginValid: function () { return this.newuser.Login.length == 0 ? null : this.newuser.Login.indexOf("@") != -1;},
    passwordValid: function () { return this.newuser.Password.length == 0 ? null : this.newuser.Password == this.repeatedPassword;},
    password2Valid: function () { return this.repeatedPassword.length == 0 ? null : this.newuser.Password == this.repeatedPassword;},
    nameValid: function () { return this.newuser.Name.length == 0 ? null : true;},
    formValid: function () { return this.loginValid === true && this.passwordValid === true  && this.nameValid === true;} 
  },
  methods: {
    addInterest() {
      if (
    ...
    removeInterest(interest: string) {
      this.newuser.Interests.pop(interest);
    },
    async onSubmit(evt: Event) {
      const Api = new UserService();
      let result = await Api.Register(this.newuser);
      if(result) {
        this.$router.push('/');
      }
      else {
